...
df <- filter(df, population_main %in% c("endo", "no_endo"))
...
#--------------------------
# Remove diag_endo_primary as a precaution to not use this as the outcome by mistake
#--------------------------

df_relevel <- df_relevel %>%
  select(-diag_endo_primary)

# Check counts
df_relevel %>% group_by(population_main_split, diag_endo) %>% count() %>% print()
